...
Follow.prototype.validate = async function(action) {
  ...
  if(action == "delete") {
    if(!doesFollowAlreadyExist) this.errors.push("you are cannot stop following someone, you do not already follow.")
  }
...
Follow.getFollowingById = function(id) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      following = following.map(function(follows) {
        let user = new User(follows, true)
        return {username: follows.username, avatar: user.avatar}
      })
      resolve(following)
    } catch {
      reject()
